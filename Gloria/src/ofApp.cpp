...
void ofApp::setup() {
	...
	ofAddListener(directory.events.serverAnnounced, this,
				  &ofApp::serverAnnounced);
...
void ofApp::serverAnnounced(ofxSyphonServerDirectoryEventArgs &arg) {
	for (auto &dir : arg.servers) {
		ofLogNotice("ofxSyphonServerDirectory Server Announced")
		<< " Server Name: " << dir.serverName << " | App Name: " << dir.appName;
		bool found = false;
		
		for (int i = 0; i < syphonInputs.size(); i++) {
			if (syphonInputs[i].getServerName() == dir.serverName &&
				syphonInputs[i].getApplicationName() == dir.appName) {
...
void ofApp::serverUpdated(ofxSyphonServerDirectoryEventArgs &arg) {
	for (auto &dir : arg.servers) {
		ofLogNotice("ofxSyphonServerDirectory Server Updated")
		<< " Server Name: " << dir.serverName << " | App Name: " << dir.appName;
		for (int i = 0; i < syphonInputs.size(); i++) {
			if (syphonInputs[i].getServerName() == dir.serverName &&
				syphonInputs[i].getApplicationName() == dir.appName) {
...
void ofApp::serverRetired(ofxSyphonServerDirectoryEventArgs &arg) {
	for (auto &dir : arg.servers) {
		ofLogNotice("ofxSyphonServerDirectory Server Retired")
		<< " Server Name: " << dir.serverName << " | App Name: " << dir.appName;
...
void ofApp::draw() {
	...
	for (auto s : scenes) {
		...
		for (auto syphon : syphonInputs) {
			if (syphon.getServerName() == s->name) {
				ofSetColor(255, 255, 255, 255);
				if (syphonIn->getServerName() == syphon.getServerName() &&
	...
	/*ofPushMatrix();{
	 ...
	 ofDrawRectangle(-1, -1, scenes[i]->fbo.getWidth()*scale+2,
	 scenes[i]->fbo.getHeight()*scale+2);
	 // fboOut.draw(0, 0);
	 ofSetColor(255,255,255,255);
	 if(scenes[i]->enabled) {
	 scenes[i]->fbo.draw(0,0, scenes[i]->fbo.getWidth()*scale,
	 scenes[i]->fbo.getHeight()*scale);
	 }
	 ofSetColor(255);
	...
	ofSetColor(255);
	ofDrawBitmapString("FPS: " + ofToString(ofGetFrameRate()), ofGetWidth() - 200,
					   20);
	
	/*if(mapping->selectedCorner) {
	 ofDrawBitmapString("Selected Corner: " +
	 ofToString(mapping->selectedCorner->uid) + " pos: " +
	 ofToString(mapping->selectedCorner->pos), ofGetWidth()-600, 20);
	 }*/
	...
	/*for(int i=0; i<syphonInputs.size(); i++) {
	 ...
	 syphonInputs[i].getApplicationName() + " " + syphonInputs[i].getServerName(),
	 0, 0);
...
//------------------------------------------------------------
void ofApp::debugDraw() { mapping->debugDraw(); }
	...
	if (key == 'c') {
		if (lastChangedParam != nullptr) {
			ofGetWindowPtr()->setClipboardString(
												 ofxParameterFader::getOscAddressForParameter(*lastChangedParam));
		}
	}
...
void ofApp::exit() {
	...
	for (int i = 0; i < scenes.size(); i++) {
		settings.setValue(scenes[i]->name + "_panel_pos_x",
						  scenes[i]->panel.getPosition().x);
		settings.setValue(scenes[i]->name + "_panel_pos_y",
						  scenes[i]->panel.getPosition().y);
		scenes[i]->exit();
	}
